...
//!
//! The **primary** regression net for this feature is the offline, deterministic
//! matrix in `crates/ironclaw_llm/src/mistral/tests.rs`: it already proves —
//! with loopback mock servers, no API key — that `reasoning_effort` is sent for
//! small/medium and omitted for large/off, that the array
//! (`[{thinking},{text}]`) and string responses both parse, the error mapping,
//! and that a prior turn's thinking (+ signature) is replayed as a chunk. These
//! live tests are a thin smoke layer over the one thing the offline matrix
//! cannot reach: that a *real* Mistral reasoning response round-trips through
//! the agent loop without the original `JsonError: did not match any variant of
//! untagged enum ApiResponse` parse failure, and that a second turn does not 400
//! when the parsed thinking chunk is replayed.
//!
//! ## What we assert (and what we deliberately don't)
//!
//! The verdict is **a non-empty reply with no failure signature**. We do NOT
//! assert on `StatusUpdate::Thinking`: on the v1 agent path those events are
//! reused for generic status, so their presence is not a reasoning signal.
//! Reasoning *parsing* is proven deterministically offline; the multi-turn test
//! exercises the replay round-trip end-to-end.
//!
//! ## Running
//!
//! ```bash
...
//! ```
//!
//! Outside `IRONCLAW_LIVE_TEST=1` (the default `cargo test` matrix and CI) the
//! harness resolves to `TestMode::Skipped` and the test does no network work and
//! needs no key.
...
mod live_tests {
    ...
    /// Substrings that mean the Mistral round-trip broke — the original parse
    /// bug plus the sanitized channel-boundary phrasings the agent surfaces for
    /// a provider failure (per `.claude/rules/error-handling.md`).
    ...
    /// Live-tier gate. The harness resolves `Config::from_env()` and has no
    /// backend override, so the runner must explicitly select Mistral — the same
    /// runner-sets-the-env pattern `e2e_live_reasoning.rs` uses. This guarantees
    /// we never silently run against whatever backend is configured.
    ...
    /// Point `Config::from_env()` at libSQL so config resolution doesn't demand
    /// a Postgres `DATABASE_URL`. The rig always runs against its own fresh temp
    /// libSQL DB, so this only satisfies config validation. Written through the
    /// runtime overlay `Config::from_env()` reads, not unsafe `set_var`, and only
    /// when the runner hasn't already chosen a backend.
    ...
    /// deserializes and the agent produces a coherent answer (the exact path the
    /// original `ApiResponse` parse bug broke).
    #[tokio::test]
    ...
    /// turn 2. Confirms the live ThinkChunk replay does not 400. Offline C8 proves
    /// the builder replays the chunk; this proves the real API accepts it.
    #[tokio::test]
    #[ignore] // Live tier: requires IRONCLAW_LIVE_TEST=1 + LLM_BACKEND=mistral + MISTRAL_API_KEY
    async fn mistral_reasoning_multi_turn_replays() {
        ...
        // message (with its thinking chunk + signature) into the next request.
        ...
        assert!(
            ...
            "turn 2: expected a second response — the multi-turn thinking replay must not fail \
             (HTTP 400 on turn 2 was the guarded failure mode), got {} response(s)",
            all.len()
        );
