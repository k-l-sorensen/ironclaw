...
Every WASM tool ships a `capabilities.json` alongside its `.wasm` binary. This file is the single source of truth for what the tool is allowed to do. Capabilities live under a top-level `capabilities` key; metadata (`description`, `version`, etc.) sits at the root.
...
`capabilities.http.allowlist` controls which destinations the tool can reach via the `http_request` host function. Each entry pins a `host`, an optional `path_prefix`, and the allowed `methods`. Requests that don't match an entry are rejected by the host before any connection opens.
...
`capabilities.secrets.allowed_names` is the second half of the credential gate. A tool can only resolve secrets whose names appear in this list (glob patterns like `github_*` are allowed). Even if a credential mapping references a secret, the host will refuse to load it unless the name is allowlisted here.

### Workspace read

`capabilities.workspace.allowed_prefixes` declares which workspace path prefixes the tool can read via the `workspace_read` host function. There is no `workspace_write` host function exposed to tools — workspace mutations happen through the agent's `memory_write` tool, which carries its own dispatch and audit trail.
...
| `tool_invoke(alias, params)` | Invoke another tool by alias (capability-gated; output is scanned for leaks) |
