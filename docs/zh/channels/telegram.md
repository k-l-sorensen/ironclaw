...
| `allowed_chat_ids`              | 群组聊天 ID                 | `[]`      | 将群组处理限制为指定的 Telegram 群组聊天 ID                      |
...
使用 `allowed_chat_ids` 可以将群组参与限制为特定 Telegram 群组，同时保持私信策略独立。列表为空或省略时，所有群组保持原有行为。列表中包含群组 ID 时，来自其他群组的消息会被忽略。私信仍由 `dm_policy` 控制。

允许的群组会绕过群组消息中的 `dm_policy`，因此机器人可以要求私信配对，同时允许已批准群组中的任何人提及它。提及处理仍遵循 `respond_to_all_group_messages` 和 `bot_username`。
...
Telegram 群组聊天 ID 通常是负数。您可以在群组发送消息后从更新日志或 Telegram ID 辅助机器人读取该 ID。
