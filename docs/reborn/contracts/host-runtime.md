...
- `EnforceResourceCeiling`: for immediate invoke/resume, decomposes supported ceiling dimensions into host-owned estimate/result checks plus runtime handoffs. `max_usd` and input/output token ceilings require matching host estimates before dispatch and are re-checked against measured `ResourceUsage` after dispatch. `max_output_bytes` is enforced after redaction before publication, reports the same output-limit failure category as `EnforceOutputLimit`, and is handed to process ports as a command capture cap. First-party process capabilities receive wall-clock ceilings as command timeouts; tenant sandbox process ports also receive sandbox memory and process-count ceilings for Docker host config enforcement. Sandbox CPU-time, disk, and network-egress ceilings fail closed until concrete runtime/sandbox metering exists.
