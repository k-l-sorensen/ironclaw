...
**Net:** the reported production pain (≈25.8k-token request at `message_count=5`, dominated by 91 tool schemas, timing out on NEAR AI) is addressed by **Phase 1 tool disclosure** — the one genuinely-missing capability. Phases 0/1 are committed and verified; Phases 2/3 are satisfied by existing infrastructure + measurement. Remaining before enabling disclosure in production: negative-path bridge tests, then a flagged canary validated against the Phase-0 shadow logs (real token delta + whether `cached_tokens` is populated) with the PinchBench ≥ 0.768 gate.
