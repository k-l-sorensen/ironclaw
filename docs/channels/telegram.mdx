...
Allowed groups bypass `dm_policy` for group messages, so a bot can require pairing for DMs while still allowing anyone in an approved group to mention it. Mention handling still follows `respond_to_all_group_messages` and `bot_username`.
