...
//! `contract_test_indexed!` invocation expands to one `#[tokio::test]` per
//! contract, named `in_memory::<contract_name>` so failures attribute
//! cleanly to this impl.

use ironclaw_memory::{InMemoryMemoryDocumentRepository, contract_test_indexed};

contract_test_indexed!(in_memory, InMemoryMemoryDocumentRepository::new);
