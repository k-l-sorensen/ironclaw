...
use crate::descriptors::{
    ...
    WEBUI_V2_PATTERN_CONNECTORS_CONNECTED, WEBUI_V2_PATTERN_CONNECTORS_READ,
    WEBUI_V2_PATTERN_CONNECTORS_WRITE, WEBUI_V2_PATTERN_CREATE_THREAD,
...
pub fn webui_v2_router_with_options(state: WebUiV2State, options: WebUiV2RouteOptions) -> Router {
    let mut router = Router::new()
        ...
        .route(
            ...
            get(handlers::connectors_connected),
        )
        .route(
            WEBUI_V2_PATTERN_CONNECTORS_READ,
            post(handlers::connectors_read),
        )
        .route(
            WEBUI_V2_PATTERN_CONNECTORS_WRITE,
            post(handlers::connectors_write),
        );
