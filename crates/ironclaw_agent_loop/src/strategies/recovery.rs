...
/// this strategy:
/// - Aborts immediately on `Permanent`, `InputInvalid`, `PolicyDenied`,
///   `ContextOverflow`, and `ContentFiltered` (no retry will help).
...
///
/// All capability error classes collapse to `CapabilityProtocolError` — the
/// loop-side classification is intentionally coarse; finer-grained provenance
/// lives on the diagnostic ref attached to the `CapabilityErrorSummary`.
...
mod tests {
    ...
    mod default_recovery_strategy {
        ...
        #[tokio::test]
        async fn capability_policy_denied_aborts_immediately() {
            let strategy = DefaultRecoveryStrategy::default();
            let state = state_with_attempts(0);
