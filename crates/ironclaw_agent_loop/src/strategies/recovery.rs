...
mod tests {
    ...
    mod default_recovery_strategy {
        ...
        fn state_with_attempts(attempts: u32) -> LoopExecutionState {
            state_with_attempts_for(attempts, None)
        }

        fn state_with_attempts_for(
            attempts: u32,
            attempt_class: Option<RecoveryAttemptClass>,
        ) -> LoopExecutionState {
            let mut state = LoopExecutionState::initial_for_run(&test_run_context());
            state.recovery_state = RecoveryStrategyState {
                attempts,
                attempt_class,
            };
            state
        ...
        async fn capability_transient_retries_then_aborts_at_budget() {
            ...
            let state = state_with_attempts_for(2, Some(RecoveryAttemptClass::CapabilityTransient));
            let outcome = strategy
                .on_capability_error(&state, &cap_err(CapabilityErrorClass::Transient))
                .await;
            assert!(matches!(
        ...
        async fn model_context_overflow_retries_with_context_shrink_then_aborts_at_budget() {
            ...
            match outcome {
                RecoveryOutcome::Retry {
                    ...
                    assert_eq!(
                        recovery.attempt_class,
                        Some(RecoveryAttemptClass::ModelContextOverflow)
                    );
            ...
            let state =
                state_with_attempts_for(2, Some(RecoveryAttemptClass::ModelContextOverflow));
            let outcome = strategy
                .on_model_error(&state, &model_err(ModelErrorClass::ContextOverflow))
                .await;
            assert!(matches!(
        ...
        async fn model_transient_retries_then_aborts_at_budget() {
            ...
            let state = state_with_attempts_for(2, Some(RecoveryAttemptClass::ModelTransient));
            let outcome = strategy
                .on_model_error(&state, &model_err(ModelErrorClass::Transient))
                .await;
            assert!(matches!(outcome, RecoveryOutcome::Abort { .. }));
        ...
        #[tokio::test]
        async fn retry_budget_resets_when_error_class_changes() {
            let strategy = DefaultRecoveryStrategy::default();
            let state = state_with_attempts_for(2, Some(RecoveryAttemptClass::CapabilityTransient));
        ...
        async fn non_retry_paths_do_not_poison_later_retry_budget() {
            ...
            assert!(matches!(
                ...
                RecoveryOutcome::Retry {
                    recovery: RecoveryStrategyState {
                        attempts: 1,
                        attempt_class: Some(RecoveryAttemptClass::ModelTransient),
                    },
                    ..
                }
            ));
