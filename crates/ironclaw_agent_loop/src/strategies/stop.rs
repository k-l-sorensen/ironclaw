...
impl StopConditionStrategy for DefaultStopConditionStrategy {
    async fn should_stop_after_turn(
        ...
        let next = ControlStrategyState {
            turns_completed: state.control_state.turns_completed.saturating_add(1),
...
mod tests {
    ...
    mod default_stop_condition_strategy {
        ...
        use crate::state::{CapabilityCallSignature, ControlStrategyState, LoopExecutionState};
        ...
        async fn no_signal_continues_with_turns_completed_incremented() {
            ...
            state.control_state = ControlStrategyState {
                turns_completed: 4,
            ...
            match outcome {
                StopOutcome::Continue { control } => {
                    assert_eq!(control.turns_completed, 5);
                }
        ...
        async fn all_results_terminate_hint_returns_graceful_stop() {
            ...
            match outcome {
                StopOutcome::Stop { control, kind } => {
                    assert_eq!(control.turns_completed, 2);
