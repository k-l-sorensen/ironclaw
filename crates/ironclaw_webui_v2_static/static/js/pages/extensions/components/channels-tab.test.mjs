...
function channelsTabSourceForTest() {
  ...
  return `${lines.join("\n")}\nglobalThis.__testExports = { ChannelsTab, SlackConnectActionSections, isSlackChannelEnabled, slackBuiltinStatus, isSlackPackage, isSlackAdminManagedAction, isSlackInboundProofCodeAction, isConnectedChannel, shouldRenderSlackPairingSection, findSlackConnectAction, findSlackConnectActions };`;
}
...
test("SlackConnectActionSections renders every supported Slack action", () => {
  ...
  const connectedView = slackConnectActionSectionsForTest({
    channel: "slack",
    strategy: "inbound_proof_code",
    connection_status: "connected",
    action: {},
