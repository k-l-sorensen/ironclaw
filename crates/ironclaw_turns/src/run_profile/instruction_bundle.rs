...
impl InstructionBundleBuilder {
    ...
    pub fn build(
        ...
        for (ordinal, message) in request.context_bundle.identity_messages.into_iter().enumerate()
        {
...
fn push_context_message(
    ...
    let (content_ref, summary_only) = match source_ref {
        ...
        None => {
            let summary_section = if section == "identity" {
                "identity-summary"
            } else {
                "context-summary"
            };
    ...
    if force_materialize || summary_only {
