...
fn validation_error_input_issues(
    ...
    let (path, code, expected, received) = match error.kind().keyword() {
        ...
        "additionalProperties" | "unevaluatedProperties" => {
            return unexpected_field_issues(arguments, schema, error);
        }
        "type" => (
...
fn unexpected_field_issues(
    ...
    let Some(object) = value_at_json_pointer(arguments, instance_path).and_then(|v| v.as_object())
        ...
        return vec![unexpected_field_issue(parent_path, schema_path)];
    };
...
fn missing_required_properties(
    ...
    let Some(schema_object) = schema_at_required_parent(schema, error.schema_path().as_str())
    else {
        return Vec::new();
    };
...
fn schema_at_required_parent<'a>(
    schema: &'a serde_json::Value,
    schema_path: &str,
) -> Option<&'a serde_json::Map<String, serde_json::Value>> {
    schema_at_keyword_parent(schema, schema_path, "required")
}
